...
if "내 쇼핑몰" in display_title or "종합 대시보드" in display_title:
    st.markdown(f'<div class="dashboard-title">{display_title}</div>', unsafe_allow_html=True)
else:
...
with col_main:
    # 0. 종합 대시보드 (메인 홈)
    if menu == "🏠 종합 대시보드":
        st.markdown("### 🏠 쇼핑몰 종합 대시보드")
        
        is_preset = st.session_state.get("diagnose_is_preset", False)
        if is_preset:
            st.info("💡 **현재 예시 데이터(올라운드 가구숍)로 대시보드가 로드되었습니다.** 내 쇼핑몰의 실제 실적 파일(.xlsx / .csv)을 업로드하려면 왼쪽 메뉴의 **'📦 내 쇼핑몰 상품 진단'**으로 이동해 주세요.")
        else:
            st.success("✅ **성공적으로 업로드된 쇼핑몰 실적 데이터로 분석 중입니다.**")
            
        df_cleaned = st.session_state.get("diagnose_df")
        ...
        if df_cleaned is not None:
            ...
            # 그린 톤 마케팅 스코어카드 상단 배치
            st.markdown("#### 🟢 쇼핑몰 핵심 성과 지표")
            col_dash1, col_dash2, col_dash3, col_dash4 = st.columns(4)
            with col_dash1:
                make_green_card("🎯 총 유입수 (클릭)", f"{total_clicks:,}회", "전체 상품 누적 클릭수")
            with col_dash2:
                make_green_card("🛍️ 총 구매 건수", f"{total_purchases:,}건", "전체 상품 누적 주문건수")
            with col_dash3:
                make_green_card("💰 총 매출액", f"{total_revenue:,}원", "전체 상품 누적 매출")
            with col_dash4:
                make_green_card("📈 평균 구매전환율 (CVR)", f"{overall_cvr:.2f}%", "총 구매수 / 총 클릭수")
            ...
            with col_c2:
                ...
                fig_top10.update_traces(marker_cornerradius=12)
                st.plotly_chart(fig_top10, use_container_width=True)
                
            st.markdown("---")
            
            # 액션플랜 체크리스트 & 게이지 (2열)
            col_g, col_chk = st.columns([2, 3])
            ...
            with col_g:
                ...
                fig_gauge.update_layout(
                    ...
                    annotations=[
                        dict(
                            text=f"<span style='font-size:24px; font-weight:bold; color:#10b981;'>{completion_rate:.0f}%</span><br><span style='font-size:11px; color:#9ca3af;'>달성률</span>",
                            x=0.5, y=0.55,
                            showarrow=False,
                            align="center"
                        )
            ...
            with col_chk:
                ...
                st.markdown(" ")
                if st.button("🔄 액션 플랜 체크 상태 초기화", use_container_width=True, key="home_reset_actions_btn"):
                    st.session_state["diagnose_actions"] = {}
                    st.rerun()
                    
            st.markdown("---")
            
            # 비즈니스 핵심 진단 리포트 요약 카드
            st.markdown("#### 💡 비즈니스 핵심 진단 요약 리포트")
